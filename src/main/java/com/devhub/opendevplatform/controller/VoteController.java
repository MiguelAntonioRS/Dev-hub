...
public class VoteController {
    ...
    public String upvote(@PathVariable Long resourceId, @RequestParam Long userId) {
    ...
    public String downvote(@PathVariable Long resourceId, @RequestParam Long userId) {
        User user = userService.findById(userId).orElseThrow();
        Resource resource = resourceService.findById(resourceId).orElseThrow();
        voteService.voteResource(user, resource, -1);
        return "redirect:/resources";
